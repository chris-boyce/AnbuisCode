...
public class LeverCheck1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (levercheck1 == 1)// Used to make sure running the right part of the script
            {

                if (Input.GetKeyUp("e"))// input buttin
                {
                    rb.transform.Rotate(180, 0, 0);
                    levercheck1 = 0; // setting value to 0 for LeverCheck.cs to add to make postive outcome
                }
            }
