...
public class LeverCheck2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (levercheck2 == 1)// Used to make sure running the right part of the script
            {

                if (Input.GetKeyUp("e"))// input buttin
                {
                    rb.transform.Rotate(180, 0, 0);
                    levercheck2 = 0; // setting value to 0 for LeverCheck.cs to add to make postive outcome
                }
            }
